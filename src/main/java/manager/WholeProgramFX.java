...
public class WholeProgramFX implements SingleInstanceFX<WholeProgramFeatureSet, WholeProgramFEU> {
    ...
    public WholeProgramFeatureSet getFeatures(Set<WholeProgramFEU> featureExtractors) {
        ...
        Set<SootMethod> publicMethods = classes.stream().flatMap(cls -> cls.getMethods().stream()
                .filter(m -> m.isPublic() && m.isConcrete())).collect(Collectors.toSet());
    ...
    @Override
    public WholeProgramFeatureSet getAllFeaturesExclude(Set<String> exclusion, List<FeatureResource> featureResources) {
        List<FeatureDescription> features = FxUtil.listAllWholeProgramFeatures();
        List<String> names = features.stream().map(f -> f.getName()).collect(Collectors.toList());
        ...
        for (String str : featureExtractors) {
            ...
            try {
                ...
                if(featureResource.isPresent()){
                    ...
                    newInstance = (WholeProgramFEU) cls.getConstructor(String.class, View.class).newInstance(featureValue, view);
                } else {
    ...
    public static Optional<FeatureResource> getResourcePath(List<FeatureResource> featureResources, String featureName) {
