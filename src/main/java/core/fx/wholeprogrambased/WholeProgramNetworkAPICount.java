...
public class WholeProgramNetworkAPICount implements WholeProgramFEU<Long> {
    ...
    public Feature<Long> extract(CallGraph target) {
        ...
        stream.forEach(edge -> {
            if (edge.tgt() != null && FxUtil.isNetworkAPIPackage(edge.tgt().getDeclaringClass().getPackageName())) {
