...
public class WholeProgramLoggingCount  implements WholeProgramFEU<Long> {
    ...
    public Feature<Long> extract(CallGraph target) {
        ...
        stream.forEach(edge -> {
            if(edge.tgt() != null && FxUtil.isLoggerPackage(edge.tgt().getDeclaringClass().getPackageName())){
