...
public class MethodCallsMethod implements MethodFEU<Boolean> {
  ...
  public boolean checkMethod(SootMethod method, List<SootMethod> doneList) {
    if (doneList.contains(method)) return false;
      ...
      for (Stmt u : body.getStmts()) {
        ...
        if (Strings.CI.startsWith(inv.getMethodSignature().getName(), this.methodName)) {
          if (this.className.isEmpty()
              || this.className.equals(inv.getMethodSignature().getDeclClassType().getClassName()))
            return true;
        } else if (this.view.getMethod(inv.getMethodSignature()).isPresent()
            && checkMethod(this.view.getMethod(inv.getMethodSignature()).get(), doneList))
          return true;
      }
      return false;
    } catch (Exception e) {
